    ...
    def updateLastDatas(self, buildingSign):
        ...
        if list_z != []:
            try:
                ...
                # self.log.error(error)
                self.mysql_conn.rollback()
                print(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')+'数据接收失败:' + error)
                # break
            else:
                self.mysql_conn.commit()
            # self.log.info(table_name + '写入成功')
        print(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')+'数据接收成功['+str(counts)+']')
...
if __name__ == '__main__':
    ...
    try:
        test = BaseFunction()
        test.get_token()
        test.updateDatas('3702822027', datetime.datetime(2022, 2, 21, 15, 0, 0), datetime.datetime(2022, 2, 24, 8, 30, 0))

        ## data = test.get_numbers('queryMeterInfo',{"pointId": "HSHL201212024015", "pageIndex": "1", "pageLimit": "100"})
        test.updateLastDatas('3702822027')
        schedule.every(5).minutes.do(test.updateLastDatas, '3702822027')
        while True:
